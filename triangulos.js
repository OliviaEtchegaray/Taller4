...
function startTriangulo3() {
    ...
    let targets = [
        { x: mainCanvas.width * 0.3, y: mainCanvas.height * 0.3, size: 50, angle: 0, matched: false, glow: 0 },
        { x: mainCanvas.width * 0.7, y: mainCanvas.height * 0.4, size: 50, angle: Math.PI * 2 / 3, matched: false, glow: 0 },
        { x: mainCanvas.width * 0.5, y: mainCanvas.height * 0.7, size: 50, angle: Math.PI * 4 / 3, matched: false, glow: 0 }
    ];
    ...
    // Envoltorio para usar el estilo de gradiente con rotación
    function drawRotatedGradientTriangle(ctx, x, y, size, angle, r, g, b, alpha) {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(angle);
        drawGradientTriangle(ctx, 0, 0, size, r, g, b, alpha);
        ctx.restore();
    }

    // Dibujo tradicional preservado para los huecos grises (siluetas)
    ...
    function animate() {
        ...
        // 1. Dibujar Huecos Grises
        targets.forEach(t => {
            let color = `rgb(${100 + t.glow * 155}, ${100 + t.glow * 155}, ${100 + t.glow * 155})`;
            drawTriangle(mainCtx, t.x, t.y, t.size, t.angle, t.matched ? color : "#cccccc");
            if (t.glow > 0) t.glow -= 0.02;
        });

        // 2. Actualizar Piezas (Ahora verdes)
        successCount = 0;
        pieces.forEach(p => {
            ...
            if (!t.matched) {
                ...
                if (dist < 15) { 
                    ...
                    p.x = t.x; 
                    p.y = t.y;
                    currentBg = (currentBg + 1) % bgColors.length; // Flash de fondo al encajar pieza
                }
            }

            if (!t.matched) {
                // Misma estética que el 1 y el 2 usando la función envoltorio
                drawRotatedGradientTriangle(mainCtx, p.x, p.y, p.size, p.angle, 50, 205, 50, 1);
            } else {
                successCount++;
            }
        });
